...
const UI_Explanation = () => {
	...
	const handleChangePopperPosition = (changeValue) => {
		...
		setAnchorEl(
			document.getElementById(`button-${currentButtonId + changeValue}`)
		);
		setOpen(true);
	};
				...
				>
					...
					>
						...
						<Typography variant="h5">
							{" "}
							{currentUI[currentButtonId - 1]?.title}{" "}
						</Typography>
						...
						>
							<Button
								onClick={() => handleChangePopperPosition(-1)}
								disabled={currentButtonId === 1}
							>
								Previous
							</Button>
							<Button
								onClick={() => handleChangePopperPosition(1)}
								disabled={currentButtonId === currentUI.length}
							>
								Next
							</Button>
						</Box>
